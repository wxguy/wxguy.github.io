...
If you want beautiful font for editor, you can install *Victor Mono* font from aur with following command:-
...
> Installing `LSP-pyrignt` itself good enough to get auto completion in Sublime Text. However, bracket completion for methods and functions such as simple `print` statement is not provided by pyright. To make it working, we have to install `Jedi` plugin as well. Important point to note here is that `Jedi` plugin is little old and almost no commit is made in the last two years. 
> {: .prompt-tip }
...
[AutoDocstring](https://packagecontrol.io/packages/AutoDocstring) plugin enable users to easily create a doc string by using key combination. After you install the plugin, go to class or function or method line (where `:` is written) and prese key comination `ctrl` + `alt` + `'`. This will automatically create the doc string template for you. You just have to move from one to another field by using tab.
